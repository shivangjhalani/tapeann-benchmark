...
void execute_io(io_context_t ctx, int fd, std::vector<AlignedRead> &read_reqs, uint64_t n_retries = 0)
{
    ...
    for (uint64_t iter = 0; iter < n_iters; iter++)
    {
        ...
        while (n_tries <= n_retries)
        {
            ...
            {
                // Drain all n_ops completions with a 30s per-call timeout.
                // The original nullptr timeout caused an infinite hang on kernel
                // 6.8 when libaio dropped a completion event (O_DIRECT + NVMe).
                // min_nr=1 lets us drain incrementally rather than blocking until
                // ALL events arrive at once, which is what triggered the deadlock.
                int64_t remaining = (int64_t)n_ops;
                struct timespec timeout = {30, 0};
                while (remaining > 0)
                {
                    ret = io_getevents(ctx, 1, remaining, evts.data() + (n_ops - remaining), &timeout);
                    if (ret < 0)
                    {
                        std::cerr << "io_getevents() failed; returned " << ret << ", ernno=" << errno
                                  << "=" << ::strerror(-ret) << ", try #" << n_tries + 1;
                        exit(-1);
                    }
                    if (ret == 0)
                    {
                        std::cerr << "io_getevents() timed out waiting for " << remaining
                                  << " of " << n_ops << " completions; exiting.\n";
                        exit(-1);
                    }
                    remaining -= ret;
                }
